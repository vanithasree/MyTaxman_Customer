...
class ClosedJobTableViewCell: UITableViewCell {
    ...
    func setCellViewUI() {
        ...
        statusLabel.setLabelCustomProperties(titleText: "Expired", textColor: ColorManager.darkText.color, font:  UIFont(name:Font.FontName.PoppinsRegular.rawValue, size: Utility.dynamicSize(12.0)), numberOfLines: 1, alignment: .left)
    }
    
    func setValue(data: ClosedJobListDesc){
        jobNameLabel.text = data.category ?? ""
        commentsLabel.text = data.cancel_description ?? ""
    }
}
